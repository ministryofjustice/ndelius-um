...
public class UserTransformer
{
	...
	public ExportResult combine(UserExportEntity db, UserEntry ldap) {
		...
		return ExportResult.builder()
				.username(ofNullable(ldap).map(UserEntry::getUsername).orElseGet(db::getUsername))
				.forenames(ofNullable(ldap).map(UserEntry::getForenames).orElseGet(() -> combineNames(db.getForename(), db.getForename2())))
				.surname(ofNullable(ldap).map(UserEntry::getSurname).orElseGet(db::getSurname))
				.email(ofNullable(ldap).map(UserEntry::getEmail).orElse(null))
				.telephoneNumber(ofNullable(ldap).map(UserEntry::getTelephoneNumber).orElse(null))
				.homeArea(ofNullable(ldap).map(UserEntry::getHomeArea).orElse(null))
				.endDate(ofNullable(ldap).map(UserEntry::getEndDate).map(LdapUtils::mapLdapStringToDate).orElseGet(db::getEndDate))
				.startDate(ofNullable(ldap).map(UserEntry::getStartDate).map(LdapUtils::mapLdapStringToDate).orElseGet(() -> ofNullable(db.getStaff()).map(StaffExportEntity::getStartDate).orElse(null)))
				.sector(ofNullable(ofNullable(ldap).map(UserEntry::getSector).map("private"::equalsIgnoreCase).orElseGet(db::getPrivateUser)).map(p -> p ? "Private" : "Public").orElse(null))
