...
public interface SearchResultRepository extends JpaRepository<SearchResultEntity, String>
{
	@Query(value = "SELECT U.DISTINGUISHED_NAME AS DISTINGUISHED_NAME,"
			...
			+ "		SYS.UTL_MATCH.EDIT_DISTANCE_SIMILARITY(LOWER(T.DESCRIPTION), LOWER(?1)))) / 100 AS SCORE"
			+ "	FROM USER_ U"
			...
			+ "		OR CASE WHEN U.FORENAME IS NULL THEN LENGTH(?1) ELSE SYS.UTL_MATCH.EDIT_DISTANCE(LOWER(U.FORENAME), LOWER(?1)) END <= 2"
			+ "		OR CASE WHEN U.FORENAME2 IS NULL THEN LENGTH(?1) ELSE SYS.UTL_MATCH.EDIT_DISTANCE(LOWER(U.FORENAME2), LOWER(?1)) END <= 2"
			+ "		OR CASE WHEN U.SURNAME IS NULL THEN LENGTH(?1) ELSE SYS.UTL_MATCH.EDIT_DISTANCE(LOWER(U.SURNAME), LOWER(?1)) END <= 2"
			+ "		OR CASE WHEN S.OFFICER_CODE IS NULL THEN LENGTH(?1) ELSE SYS.UTL_MATCH.EDIT_DISTANCE(LOWER(S.OFFICER_CODE), LOWER(?1)) END <= 2"
			+ "		OR CASE WHEN T.CODE IS NULL THEN LENGTH(?1) ELSE SYS.UTL_MATCH.EDIT_DISTANCE(LOWER(T.CODE), LOWER(?1)) END <= 2"
			+ "		OR CASE WHEN T.DESCRIPTION IS NULL THEN LENGTH(?1) ELSE SYS.UTL_MATCH.EDIT_DISTANCE(LOWER(T.DESCRIPTION), LOWER(?1)) END <= 2"
	...
	@Query(value = "SELECT U.DISTINGUISHED_NAME, "
			...
			+ "      CASE WHEN U.FORENAME IS NULL THEN 0 ELSE LENGTH(?1)/LENGTH(U.FORENAME) END, "
			+ "      CASE WHEN U.FORENAME2 IS NULL THEN 0 ELSE LENGTH(?1)/LENGTH(U.FORENAME2) END, "
			+ "      CASE WHEN U.SURNAME IS NULL THEN 0 ELSE LENGTH(?1)/LENGTH(U.SURNAME) END, "
			+ "      CASE WHEN S.OFFICER_CODE IS NULL THEN 0 ELSE LENGTH(?1)/LENGTH(S.OFFICER_CODE) END, "
			+ "      CASE WHEN T.CODE IS NULL THEN 0 ELSE LENGTH(?1)/LENGTH(T.CODE) END, "
			+ "      CASE WHEN T.DESCRIPTION IS NULL THEN 0 ELSE LENGTH(?1)/LENGTH(T.DESCRIPTION) END)) AS SCORE "
			+ "FROM USER_ U "
			+ "      LEFT OUTER JOIN STAFF S on U.STAFF_ID = S.STAFF_ID "
			+ "      LEFT OUTER JOIN STAFF_TEAM ST on U.STAFF_ID = ST.STAFF_ID "
			+ "      LEFT OUTER JOIN TEAM T ON ST.TEAM_ID = T.TEAM_ID "
