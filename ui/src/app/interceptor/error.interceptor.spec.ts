...
describe('ErrorInterceptor', () => {
  ...
  it('should parse validation errors with no body', () => {
    ...
    expect(message).toEqual('<h5>400 Bad Request</h5>');
  });
  ...
  it('should parse authorisation failures', () => {
    ...
    expect(message).toEqual('<h5>403 Forbidden</h5>Access denied. Missing roles: ROLE1, ROLE2');
  });
  ...
  it('should handle unknown error codes', () => {
    ...
    expect(message).toEqual('<h5>999 Unknown</h5>');
  });
  ...
  it('should intercept error responses', (done) => {
    new ErrorInterceptor().intercept(new HttpRequest<any>('GET', 'someurl'), {
      ...
      expect(AppComponent.globalMessage).toBe('<h5>500 Internal Server Error</h5>');
      done();
    });
  });
